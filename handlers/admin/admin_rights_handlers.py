...
from states.menu_states import AdminRegistrationState
...
async def show_baristas_to_admin(call: CallbackQuery, state: FSMContext, role: str):
    ...
    await state.set_state(AdminRegistrationState.wait_name)
...
async def approve_admin(call: CallbackQuery, state: FSMContext, role: str):
    ...
    print(user_id)
    await call.message.edit_text(text='Подтвердите выбор:', reply_markup=yes_or_no_btn())
...
async def save_role_admin(call: CallbackQuery, state: FSMContext, role: str):
    ...
    if call.data == "yes":
        try:
            ...
            await call.message.answer(text='Сохранено', reply_markup=admin_rights()
            )
